...
class SelfAttentionLayerem(nn.Module):
    def __init__(
        self,adaptive_embedding_dim,model_dim, feed_forward_dim=2048, num_heads=8, dropout=0, mask=False
    ):
        super().__init__()
        self.weigh2 = nn.Linear(in_features=adaptive_embedding_dim, out_features=model_dim)
        self.attn = AttentionLayer(model_dim, num_heads, mask)
        self.feed_forward = nn.Sequential(
            nn.Linear(model_dim, feed_forward_dim),
            nn.ReLU(inplace=True),
...
class ASPMFormer(nn.Module):
    def __init__(
        ...
        self.attn_layers_t = nn.ModuleList([
            nn.ModuleList([
                SelfAttentionLayerem(self.adaptive_embedding_dim,self.model_dim, feed_forward_dim, num_heads, dropout),  # 第一次用的
